...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenOutlineFade : FsmStateAction
	{
		...
		{
			tweener = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Outline>().DOFade(to.Value, duration.Value);
			...
			if (debugThis.Value)
			{
				Debug.Log("GameObject [" + base.State.Fsm.GameObjectName + "] FSM [" + base.State.Fsm.Name + "] State [" + base.State.Name + "] - DOTween Outline Fade - SUCCESS!");
			}
			if (finishImmediately.Value)
			{
				Finish();
