...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class ProjectLocationToMap : FsmStateAction
	{
		...
		private void DoProjectGPSLocation()
		{
			...
			x *= width.Value;
			y *= height.Value;
			projectedX.Value = ((!normalized.Value) ? (minX.Value + x * (float)Screen.width) : (minX.Value + x));
			projectedY.Value = ((!normalized.Value) ? (minY.Value + y * (float)Screen.height) : (minY.Value + y));
		}
