...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenCameraFarClipPlane : FsmStateAction
	{
		...
		{
			tweener = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Camera>().DOFarClipPlane(to.Value, duration.Value);
			...
			if (debugThis.Value)
			{
				Debug.Log("GameObject [" + base.State.Fsm.GameObjectName + "] FSM [" + base.State.Fsm.Name + "] State [" + base.State.Name + "] - DOTween Camera Far Clip Plane - SUCCESS!");
			}
			if (finishImmediately.Value)
			{
				Finish();
