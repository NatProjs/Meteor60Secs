...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenTextFade : FsmStateAction
	{
		...
		{
			tweener = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Text>().DOFade(to.Value, duration.Value);
			...
			if (debugThis.Value)
			{
				Debug.Log("GameObject [" + base.State.Fsm.GameObjectName + "] FSM [" + base.State.Fsm.Name + "] State [" + base.State.Name + "] - DOTween Text Fade - SUCCESS!");
			}
			if (finishImmediately.Value)
			{
				Finish();
