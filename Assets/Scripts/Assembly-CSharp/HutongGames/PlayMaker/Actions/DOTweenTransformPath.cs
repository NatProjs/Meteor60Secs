...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenTransformPath : FsmStateAction
	{
		...
		public override void OnEnter()
		{
			...
			{
			case LookAt.nothing:
				tweener = component.DOPath(array, duration.Value, pathType, pathMode, resolution.Value, gizmoColor.Value).SetOptions(closePath.Value, lockPosition, lockRotation);
				break;
			case LookAt.position:
				tweener = component.DOPath(array, duration.Value, pathType, pathMode, resolution.Value, gizmoColor.Value).SetOptions(closePath.Value, lockPosition, lockRotation).SetLookAt(lookAtPosition.Value, forwardDirection.Value, up.Value);
				break;
			case LookAt.target:
				tweener = component.DOPath(array, duration.Value, pathType, pathMode, resolution.Value, gizmoColor.Value).SetOptions(closePath.Value, lockPosition, lockRotation).SetLookAt(lookAtTarget.Value.transform, forwardDirection.Value, up.Value);
				break;
			case LookAt.ahead:
				tweener = component.DOPath(array, duration.Value, pathType, pathMode, resolution.Value, gizmoColor.Value).SetOptions(closePath.Value, lockPosition, lockRotation).SetLookAt(lookAhead.Value, forwardDirection.Value, up.Value);
				break;
			}
			...
			if (debugThis.Value)
			{
				Debug.Log("GameObject [" + base.State.Fsm.GameObjectName + "] FSM [" + base.State.Fsm.Name + "] State [" + base.State.Name + "] - DOTween Transform Path - SUCCESS!");
			}
			if (finishImmediately.Value)
			{
				Finish();
