...
namespace Com.LuisPedroFonseca.ProCamera2D
{
	...
	public class ProCamera2DRooms : BasePC2D, IPositionOverrider, ISizeOverrider
	{
		...
		public Room CurrentRoom
		{
			get
			{
				return (_currentRoomIndex < 0 || _currentRoomIndex >= Rooms.Count) ? null : Rooms[_currentRoomIndex];
			}
		}
		...
		public void AddRoom(float roomX, float roomY, float roomWidth, float roomHeight, float transitionDuration = 1f, EaseType transitionEaseType = EaseType.EaseInOut, bool scaleToFit = false, bool zoom = false, float zoomScale = 1.5f, string id = "")
		{
			Room room = new Room();
			room.ID = id;
			room.Dimensions = new Rect(roomX, roomY, roomWidth, roomHeight);
			room.TransitionDuration = transitionDuration;
			room.TransitionEaseType = transitionEaseType;
			room.ScaleCameraToFit = scaleToFit;
			room.Zoom = zoom;
			room.ZoomScale = zoomScale;
			Room item = room;
			Rooms.Add(item);
		}
		...
		private void TransitionToRoom(Room room, bool useTransition = true)
		{
			...
			_transitionRoutine = StartCoroutine(TransitionRoutine(numericBoundariesSettings2, num, (!useTransition) ? 0f : room.TransitionDuration, room.TransitionEaseType));
		}
		...
		private IEnumerator TransitionRoutine(NumericBoundariesSettings numericBoundariesSettings, float targetSize, float transitionDuration = 1f, EaseType transitionEaseType = EaseType.EaseOut)
		{
			...
			while (t <= 1f)
			{
				...
				float targetPosH = base.ProCamera2D.CameraTargetPositionSmoothed.x;
				float targetPosV = base.ProCamera2D.CameraTargetPositionSmoothed.y;
				LimitToNumericBoundaries(ref targetPosH, ref targetPosV, targetSize * base.ProCamera2D.GameCamera.aspect, targetSize, numericBoundariesSettings);
				float newPosH = Utils.EaseFromTo(initialCamPosH, targetPosH, t, transitionEaseType);
				float newPosV = Utils.EaseFromTo(initialCamPosV, targetPosV, t, transitionEaseType);
