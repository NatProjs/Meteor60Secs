...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenLightBlendableColor : FsmStateAction
	{
		...
		{
			tweener = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Light>().DOBlendableColor(to.Value, duration.Value);
			...
			if (debugThis.Value)
			{
				Debug.Log("GameObject [" + base.State.Fsm.GameObjectName + "] FSM [" + base.State.Fsm.Name + "] State [" + base.State.Name + "] - DOTween Light Blendable Color - SUCCESS!");
			}
			if (finishImmediately.Value)
			{
				Finish();
