...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenLightShadowStrength : FsmStateAction
	{
		...
		{
			tweener = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Light>().DOShadowStrength(to.Value, duration.Value);
			...
			if (debugThis.Value)
			{
				Debug.Log("GameObject [" + base.State.Fsm.GameObjectName + "] FSM [" + base.State.Fsm.Name + "] State [" + base.State.Name + "] - DOTween Light Shadow Strength - SUCCESS!");
			}
			if (finishImmediately.Value)
			{
				Finish();
