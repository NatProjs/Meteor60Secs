...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenMaterialFade : FsmStateAction
	{
		...
		{
			tweener = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Renderer>().material.DOFade(to.Value, duration.Value);
			...
			if (debugThis.Value)
			{
				Debug.Log("GameObject [" + base.State.Fsm.GameObjectName + "] FSM [" + base.State.Fsm.Name + "] State [" + base.State.Name + "] - DOTween Material Fade - SUCCESS!");
			}
			if (finishImmediately.Value)
			{
				Finish();
