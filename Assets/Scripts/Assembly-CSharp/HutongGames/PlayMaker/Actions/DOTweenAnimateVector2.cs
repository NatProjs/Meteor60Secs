...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenAnimateVector2 : FsmStateAction
	{
		...
		{
			tweener = DOTween.To(() => variable.Value, delegate(Vector2 x)
			{
				variable.Value = x;
			...
			if (debugThis.Value)
			{
				Debug.Log("GameObject [" + base.State.Fsm.GameObjectName + "] FSM [" + base.State.Fsm.Name + "] State [" + base.State.Name + "] - DOTween Animate Vector2 - SUCCESS!");
			}
			if (finishImmediately.Value)
			{
				Finish();
