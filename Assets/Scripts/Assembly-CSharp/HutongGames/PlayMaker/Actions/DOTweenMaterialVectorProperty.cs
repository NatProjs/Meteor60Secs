...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenMaterialVectorProperty : FsmStateAction
	{
		...
		{
			tweener = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Renderer>().material.DOVector(to.Value, property.Value, duration.Value);
			...
			if (debugThis.Value)
			{
				Debug.Log("GameObject [" + base.State.Fsm.GameObjectName + "] FSM [" + base.State.Fsm.Name + "] State [" + base.State.Name + "] - DOTween Material Vector Property - SUCCESS!");
			}
			if (finishImmediately.Value)
			{
				Finish();
