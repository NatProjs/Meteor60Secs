...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenTransformPunchPosition : FsmStateAction
	{
		...
		{
			tweener = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Transform>().DOPunchPosition(punch.Value, duration.Value, vibrato.Value, elasticity.Value, snapping.Value);
			...
			if (debugThis.Value)
			{
				Debug.Log("GameObject [" + base.State.Fsm.GameObjectName + "] FSM [" + base.State.Fsm.Name + "] State [" + base.State.Name + "] - DOTween Transform Punch Position - SUCCESS!");
			}
			if (finishImmediately.Value)
			{
				Finish();
