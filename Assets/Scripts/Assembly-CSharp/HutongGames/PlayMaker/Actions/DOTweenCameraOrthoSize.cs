...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenCameraOrthoSize : FsmStateAction
	{
		...
		{
			tweener = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Camera>().DOOrthoSize(to.Value, duration.Value);
			...
			if (debugThis.Value)
			{
				Debug.Log("GameObject [" + base.State.Fsm.GameObjectName + "] FSM [" + base.State.Fsm.Name + "]  State [" + base.State.Name + "] - DOTween Camera Ortho Size - SUCCESS!");
			}
			if (finishImmediately.Value)
			{
				Finish();
