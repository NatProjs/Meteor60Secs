...
namespace Com.LuisPedroFonseca.ProCamera2D
{
	...
	public class ProCamera2DCameraWindow : BasePC2D, IPositionDeltaChanger
	{
		...
		public Vector3 AdjustDelta(float deltaTime, Vector3 originalDelta)
		{
			...
			{
				arg = base.ProCamera2D.CameraTargetPositionSmoothed.x - (Vector3H(_transform.localPosition) + _cameraWindowRectInWorldCoords.width / 2f + CameraWindowRect.x * ((!IsRelativeSizeAndPosition) ? 1f : base.ProCamera2D.ScreenSizeInWorldCoordinates.x));
			}
			...
			{
				arg2 = base.ProCamera2D.CameraTargetPositionSmoothed.y - (Vector3V(_transform.localPosition) + _cameraWindowRectInWorldCoords.height / 2f + CameraWindowRect.y * ((!IsRelativeSizeAndPosition) ? 1f : base.ProCamera2D.ScreenSizeInWorldCoordinates.y));
			}
		...
		{
			Vector2 vector = Vector2.Scale(new Vector2(rectNormalized.width, rectNormalized.height), (!isRelative) ? Vector2.one : rectSize);
			float x = Vector3H(transf.localPosition) - vector.x / 2f + rectNormalized.x * ((!isRelative) ? 1f : rectSize.x);
			float y = Vector3V(transf.localPosition) - vector.y / 2f + rectNormalized.y * ((!isRelative) ? 1f : rectSize.y);
