...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenRigidbody2DJump : FsmStateAction
	{
		...
		public DOTweenActionsEnums.Target target;
		...
		public override void OnEnter()
		{
			...
			switch (target)
			{
			case DOTweenActionsEnums.Target.Value:
				sequence.Append(base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Rigidbody2D>().DOJump(to.Value, jumpPower.Value, numJumps.Value, duration.Value, snapping.Value));
				break;
			case DOTweenActionsEnums.Target.GameObject:
				sequence.Append(base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Rigidbody2D>().DOJump(new Vector2(toGameObject.Value.transform.position.x, toGameObject.Value.transform.position.y), jumpPower.Value, numJumps.Value, duration.Value, snapping.Value));
				break;
			}
			switch (tweenIdType)
			{
			...
			case DOTweenActionsEnums.TweenId.UseGameObject:
				sequence.SetId(base.Fsm.GetOwnerDefaultTarget(gameObject));
				break;
			}
			...
			if (!finishImmediately.Value)
			{
				...
				{
					sequence.OnComplete(base.Finish);
				}
			}
			sequence.Play();
			if (debugThis.Value)
			{
				Debug.Log("GameObject [" + base.State.Fsm.GameObjectName + "] FSM [" + base.State.Fsm.Name + "] State [" + base.State.Name + "] - DOTween RigidBody2D Jump - SUCCESS!");
			}
			if (finishImmediately.Value)
			{
				Finish();
