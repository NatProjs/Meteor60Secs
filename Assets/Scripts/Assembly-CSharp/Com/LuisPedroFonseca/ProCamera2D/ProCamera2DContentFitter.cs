...
namespace Com.LuisPedroFonseca.ProCamera2D
{
	...
	public class ProCamera2DContentFitter : BasePC2D, ISizeOverrider
	{
		...
		{
			return base.enabled ? GetSize(ContentFitterMode) : originalSize;
		}
		...
		private static void UpdateCameraAlignment(Camera cam, bool isPillarbox, float targetAspectRatio, float horizontalAlignment, float verticalAlignment)
		{
			...
			float x = ((!isPillarbox) ? 0f : ((-0.5f + targetAspectRatio / cam.aspect * 0.5f) * horizontalAlignment));
			float y = (isPillarbox ? 0f : ((-0.5f + cam.aspect / targetAspectRatio * 0.5f) * verticalAlignment));
		...
		private static Matrix4x4 GetScissorRect(Rect targetScissor, Matrix4x4 camProjectionMatrix)
		{
			...
			Matrix4x4 matrix4x2 = Matrix4x4.TRS(new Vector3((0f - targetScissor.x) * 2f / targetScissor.width, (0f - targetScissor.y) * 2f / targetScissor.height, 0f), Quaternion.identity, Vector3.one);
			return matrix4x2 * matrix4x * camProjectionMatrix;
		}
