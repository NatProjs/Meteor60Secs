...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenTransformLookAtPosition : FsmStateAction
	{
		...
		{
			tweener = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Transform>().DOLookAt(towards.Value, duration.Value, axisConstraint, up.Value);
			...
			if (debugThis.Value)
			{
				Debug.Log("GameObject [" + base.State.Fsm.GameObjectName + "] FSM [" + base.State.Fsm.Name + "] State [" + base.State.Name + "] - DOTween Transform Look At Position - SUCCESS!");
			}
			if (finishImmediately.Value)
			{
				Finish();
