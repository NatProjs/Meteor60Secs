...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenCanvasGroupFade : FsmStateAction
	{
		...
		{
			tweener = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<CanvasGroup>().DOFade(to.Value, duration.Value);
			...
			if (debugThis.Value)
			{
				Debug.Log("GameObject [" + base.State.Fsm.GameObjectName + "] FSM [" + base.State.Fsm.Name + "]  State [" + base.State.Name + "] - DOTween Canvas Group Fade - SUCCESS!");
			}
			if (finishImmediately.Value)
			{
				Finish();
