...
namespace Com.LuisPedroFonseca.ProCamera2D
{
	...
	public class ProCamera2DPanAndZoom : BasePC2D, IPreMover
	{
		...
		private void Pan(float deltaTime)
		{
			...
			Vector3 vector2 = new Vector3(Input.mousePosition.x, Input.mousePosition.y, Mathf.Abs(Vector3D(base.ProCamera2D.LocalPosition)));
			if (UsePanByDrag && Input.GetMouseButton((int)PanMouseButton))
			{
				Vector2 normalizedInput = new Vector2(Input.mousePosition.x / (float)base.ProCamera2D.GameCamera.pixelWidth, Input.mousePosition.y / (float)base.ProCamera2D.GameCamera.pixelHeight);
				if (base.ProCamera2D.GameCamera.pixelRect.Contains(vector2) && InsideDraggableArea(normalizedInput))
				{
					Vector3 vector3 = base.ProCamera2D.GameCamera.ScreenToWorldPoint(_prevMousePosition);
