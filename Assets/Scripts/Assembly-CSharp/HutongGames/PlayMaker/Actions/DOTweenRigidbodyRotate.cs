	...
	public class DOTweenRigidbodyRotate : FsmStateAction
	{
		...
		public override void OnEnter()
		{
			...
			case DOTweenActionsEnums.Target.Value:
				tweener = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Rigidbody>().DORotate(to.Value, duration.Value, rotateMode);
				break;
			case DOTweenActionsEnums.Target.GameObject:
				tweener = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Rigidbody>().DORotate(toGameObject.Value.transform.rotation.eulerAngles, duration.Value, rotateMode);
				break;
			}
			...
			if (debugThis.Value)
			{
				Debug.Log("GameObject [" + base.State.Fsm.GameObjectName + "] FSM [" + base.State.Fsm.Name + "] State [" + base.State.Name + "] - DOTween RigidBody Rotate - SUCCESS!");
			}
			if (finishImmediately.Value)
			{
				Finish();
