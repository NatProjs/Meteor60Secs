...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenTransformShakeScale : FsmStateAction
	{
		...
		{
			tweener = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Transform>().DOShakeScale(duration.Value, strength.Value, vibrato.Value, randomness.Value);
			...
			if (debugThis.Value)
			{
				Debug.Log("GameObject [" + base.State.Fsm.GameObjectName + "] FSM [" + base.State.Fsm.Name + "] State [" + base.State.Name + "] - DOTween Transform Shake Scale - SUCCESS!");
			}
			if (finishImmediately.Value)
			{
				Finish();
