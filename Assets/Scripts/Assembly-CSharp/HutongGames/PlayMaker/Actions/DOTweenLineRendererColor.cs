...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenLineRendererColor : FsmStateAction
	{
		...
		public override void OnEnter()
		{
			...
			color.ca = startValue_color_1.Value;
			color.cb = startValue_color_2.Value;
			Color2 startValue = color;
			color = default(Color2);
			color.ca = endValue_color_1.Value;
			color.cb = endValue_color_2.Value;
			Color2 endValue = color;
			tweener = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<LineRenderer>().DOColor(startValue, endValue, duration.Value);
			...
			if (debugThis.Value)
			{
				Debug.Log("GameObject [" + base.State.Fsm.GameObjectName + "] FSM [" + base.State.Fsm.Name + "] State [" + base.State.Name + "] - DOTween Line Renderer Color - SUCCESS!");
			}
			if (finishImmediately.Value)
			{
				Finish();
