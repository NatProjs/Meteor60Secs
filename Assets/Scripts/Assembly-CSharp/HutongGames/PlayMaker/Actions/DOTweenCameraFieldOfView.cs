...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenCameraFieldOfView : FsmStateAction
	{
		...
		{
			tweener = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Camera>().DOFieldOfView(to.Value, duration.Value);
			...
			if (debugThis.Value)
			{
				Debug.Log("GameObject [" + base.State.Fsm.GameObjectName + "] FSM [" + base.State.Fsm.Name + "] State [" + base.State.Name + "] - DOTween Camera Field Of View - SUCCESS!");
			}
			if (finishImmediately.Value)
			{
				Finish();
