...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenAnimateFloat : FsmStateAction
	{
		...
		{
			tweener = DOTween.To(() => variable.Value, delegate(float x)
			{
				variable.Value = x;
			...
			if (debugThis.Value)
			{
				Debug.Log("GameObject [" + base.State.Fsm.GameObjectName + "] FSM [" + base.State.Fsm.Name + "] State [" + base.State.Name + "] - DOTween Animate Float - SUCCESS!");
			}
			if (finishImmediately.Value)
			{
				Finish();
