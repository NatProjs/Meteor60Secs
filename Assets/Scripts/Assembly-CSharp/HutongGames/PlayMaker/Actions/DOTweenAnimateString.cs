...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenAnimateString : FsmStateAction
	{
		...
		{
			tweener = DOTween.To(() => variable.Value, delegate(string x)
			{
				variable.Value = x;
			...
			if (debugThis.Value)
			{
				Debug.Log("GameObject [" + base.State.Fsm.GameObjectName + "] FSM [" + base.State.Fsm.Name + "] State [" + base.State.Name + "] - DOTween Animate Rect - SUCCESS!");
			}
			if (finishImmediately.Value)
			{
				Finish();
