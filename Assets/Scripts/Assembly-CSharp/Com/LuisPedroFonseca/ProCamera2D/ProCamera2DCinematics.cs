...
namespace Com.LuisPedroFonseca.ProCamera2D
{
	...
	public class ProCamera2DCinematics : BasePC2D, IPositionOverrider, ISizeOverrider
	{
		...
		public void AddCinematicTarget(Transform targetTransform, float easeInDuration = 1f, float holdDuration = 1f, float zoom = 1f, EaseType easeType = EaseType.EaseOut, string sendMessageName = "", string sendMessageParam = "", int index = -1)
		{
			...
			cinematicTarget.Zoom = zoom;
			cinematicTarget.EaseType = easeType;
			cinematicTarget.SendMessageName = sendMessageName;
			cinematicTarget.SendMessageParam = sendMessageParam;
			CinematicTarget item = cinematicTarget;
		...
		private void SetupLetterbox()
		{
			...
			{
				Camera[] componentsInChildren = base.ProCamera2D.gameObject.GetComponentsInChildren<Camera>();
				componentsInChildren = componentsInChildren.OrderByDescending((Camera c) => c.depth).ToArray();
				componentsInChildren[0].gameObject.AddComponent<ProCamera2DLetterbox>();
			}
