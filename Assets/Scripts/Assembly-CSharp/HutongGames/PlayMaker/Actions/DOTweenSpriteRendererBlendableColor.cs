...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenSpriteRendererBlendableColor : FsmStateAction
	{
		...
		{
			tweener = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<SpriteRenderer>().DOBlendableColor(to.Value, duration.Value);
			...
			if (debugThis.Value)
			{
				Debug.Log("GameObject [" + base.State.Fsm.GameObjectName + "] FSM [" + base.State.Fsm.Name + "] State [" + base.State.Name + "] - DOTween Sprite Renderer Blendable Color - SUCCESS!");
			}
			if (finishImmediately.Value)
			{
				Finish();
