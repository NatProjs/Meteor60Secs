...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenMaterialOffsetProperty : FsmStateAction
	{
		...
		{
			tweener = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Renderer>().material.DOOffset(to.Value, property.Value, duration.Value);
			...
			if (debugThis.Value)
			{
				Debug.Log("GameObject [" + base.State.Fsm.GameObjectName + "] FSM [" + base.State.Fsm.Name + "] State [" + base.State.Name + "] - DOTween Material Offset - SUCCESS!");
			}
			if (finishImmediately.Value)
			{
				Finish();
