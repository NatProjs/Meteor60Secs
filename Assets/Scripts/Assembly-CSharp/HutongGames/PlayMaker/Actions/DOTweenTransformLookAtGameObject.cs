...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenTransformLookAtGameObject : FsmStateAction
	{
		...
		{
			tweener = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Transform>().DOLookAt(target.Value.transform.position, duration.Value, axisConstraint, up.Value);
			...
			if (debugThis.Value)
			{
				Debug.Log("GameObject [" + base.State.Fsm.GameObjectName + "] FSM [" + base.State.Fsm.Name + "] State [" + base.State.Name + "] - DOTween Transform Look At GameObject - SUCCESS!");
			}
			if (finishImmediately.Value)
			{
				Finish();
