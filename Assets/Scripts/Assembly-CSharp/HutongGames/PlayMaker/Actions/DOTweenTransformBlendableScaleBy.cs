...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenTransformBlendableScaleBy : FsmStateAction
	{
		...
		{
			tweener = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Transform>().DOBlendableScaleBy(by.Value, duration.Value);
			...
			if (debugThis.Value)
			{
				Debug.Log("GameObject [" + base.State.Fsm.GameObjectName + "] FSM [" + base.State.Fsm.Name + "] State [" + base.State.Name + "] - DOTween Transform Blendable Scale By - SUCCESS!");
			}
			if (finishImmediately.Value)
			{
				Finish();
