...
namespace Com.LuisPedroFonseca.ProCamera2D
{
	...
	public class ProCamera2DLimitDistance : BasePC2D, IPositionDeltaChanger
	{
		...
		public Vector3 AdjustDelta(float deltaTime, Vector3 originalDelta)
		{
			...
			bool flag2 = false;
			Vector2 vector = ((!UseTargetsPosition) ? new Vector2(Vector3H(base.ProCamera2D.CameraTargetPosition), Vector3V(base.ProCamera2D.CameraTargetPosition)) : new Vector2(Vector3H(base.ProCamera2D.TargetsMidPoint), Vector3V(base.ProCamera2D.TargetsMidPoint)));
			...
			base.ProCamera2D.CameraTargetPositionSmoothed = new Vector2((!flag) ? Vector3H(base.ProCamera2D.CameraTargetPositionSmoothed) : (Vector3H(base.ProCamera2D.LocalPosition) + num), (!flag2) ? Vector3V(base.ProCamera2D.CameraTargetPositionSmoothed) : (Vector3V(base.ProCamera2D.LocalPosition) + num2));
