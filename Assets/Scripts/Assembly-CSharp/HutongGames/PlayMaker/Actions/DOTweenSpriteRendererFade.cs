...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenSpriteRendererFade : FsmStateAction
	{
		...
		{
			tweener = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<SpriteRenderer>().DOFade(to.Value, duration.Value);
			...
			if (debugThis.Value)
			{
				Debug.Log("GameObject [" + base.State.Fsm.GameObjectName + "] FSM [" + base.State.Fsm.Name + "] State [" + base.State.Name + "] - DOTween Sprite Renderer Fade - SUCCESS!");
			}
			if (finishImmediately.Value)
			{
				Finish();
