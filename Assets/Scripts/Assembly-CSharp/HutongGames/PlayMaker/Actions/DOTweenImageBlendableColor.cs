...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenImageBlendableColor : FsmStateAction
	{
		...
		{
			tweener = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Image>().DOBlendableColor(to.Value, duration.Value);
			...
			if (debugThis.Value)
			{
				Debug.Log("GameObject [" + base.State.Fsm.GameObjectName + "] FSM [" + base.State.Fsm.Name + "] State [" + base.State.Name + "] - DOTween Image Blendable Color - SUCCESS!");
			}
			if (finishImmediately.Value)
			{
				Finish();
