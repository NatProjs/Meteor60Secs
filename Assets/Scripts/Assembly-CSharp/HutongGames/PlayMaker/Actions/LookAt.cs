...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class LookAt : FsmStateAction
	{
		...
		private void DoLookAt()
		{
			...
			{
				go.transform.LookAt(lookAtPos, (!upVector.IsNone) ? upVector.Value : Vector3.up);
				if (debug.Value)
				{
		...
		public bool UpdateLookAtPosition()
		{
			...
			if (goTarget != null)
			{
				lookAtPos = (targetPosition.IsNone ? goTarget.transform.position : goTarget.transform.TransformPoint(targetPosition.Value));
			}
			else
			{
