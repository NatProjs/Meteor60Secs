	...
	public class DOTweenTransformMoveX : FsmStateAction
	{
		...
		public override void OnEnter()
		{
			...
			case DOTweenActionsEnums.Target.Value:
				tweener = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Transform>().DOMoveX(to.Value, duration.Value, snapping.Value);
				break;
			case DOTweenActionsEnums.Target.GameObject:
				tweener = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Transform>().DOMoveX(toGameObject.Value.transform.position.x, duration.Value, snapping.Value);
				break;
			}
			...
			if (debugThis.Value)
			{
				Debug.Log("GameObject [" + base.State.Fsm.GameObjectName + "] FSM [" + base.State.Fsm.Name + "] State [" + base.State.Name + "] - DOTween Transform Move X - SUCCESS!");
			}
			if (finishImmediately.Value)
			{
				Finish();
