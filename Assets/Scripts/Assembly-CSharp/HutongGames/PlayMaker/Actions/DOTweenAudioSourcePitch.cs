...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenAudioSourcePitch : FsmStateAction
	{
		...
		{
			tweener = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<AudioSource>().DOPitch(to.Value, duration.Value);
			...
			if (debugThis.Value)
			{
				Debug.Log("GameObject [" + base.State.Fsm.GameObjectName + "] FSM [" + base.State.Fsm.Name + "]  State [" + base.State.Name + "] - DOTween AudioSource Pitch - SUCCESS!");
			}
			if (finishImmediately.Value)
			{
				Finish();
