...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenRectTransformPunchAnchorPos : FsmStateAction
	{
		...
		{
			tweener = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<RectTransform>().DOPunchAnchorPos(punch.Value, duration.Value, vibrato.Value, elasticity.Value, snapping.Value);
			...
			if (debugThis.Value)
			{
				Debug.Log("GameObject [" + base.State.Fsm.GameObjectName + "] FSM [" + base.State.Fsm.Name + "] State [" + base.State.Name + "] - DOTween RectTransform Punch Anchor Pos - SUCCESS!");
			}
			if (finishImmediately.Value)
			{
				Finish();
