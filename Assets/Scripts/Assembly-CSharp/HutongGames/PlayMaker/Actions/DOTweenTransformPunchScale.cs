...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenTransformPunchScale : FsmStateAction
	{
		...
		{
			tweener = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Transform>().DOPunchScale(punch.Value, duration.Value, vibrato.Value, elasticity.Value);
			...
			if (debugThis.Value)
			{
				Debug.Log("GameObject [" + base.State.Fsm.GameObjectName + "] FSM [" + base.State.Fsm.Name + "] State [" + base.State.Name + "] - DOTween Transform Punch Scale - SUCCESS!");
			}
			if (finishImmediately.Value)
			{
				Finish();
