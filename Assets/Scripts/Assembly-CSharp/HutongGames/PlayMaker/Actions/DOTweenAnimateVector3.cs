...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenAnimateVector3 : FsmStateAction
	{
		...
		{
			tweener = DOTween.To(() => variable.Value, delegate(Vector3 x)
			{
				variable.Value = x;
			}, to.Value, duration.Value);
			...
			if (debugThis.Value)
			{
				Debug.Log("GameObject [" + base.State.Fsm.GameObjectName + "] FSM [" + base.State.Fsm.Name + "] State [" + base.State.Name + "] - DOTween Animate Vector3 - SUCCESS!");
			}
			if (finishImmediately.Value)
			{
				Finish();
