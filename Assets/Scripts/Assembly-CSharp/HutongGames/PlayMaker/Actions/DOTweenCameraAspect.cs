...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenCameraAspect : FsmStateAction
	{
		...
		{
			tweener = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Camera>().DOAspect(to.Value, duration.Value);
			...
			if (debugThis.Value)
			{
				Debug.Log("GameObject [" + base.State.Fsm.GameObjectName + "] FSM [" + base.State.Fsm.Name + "]  State [" + base.State.Name + "] - DOTween Camera Aspect - SUCCESS!");
			}
			if (finishImmediately.Value)
			{
				Finish();
