...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenLightColor : FsmStateAction
	{
		...
		{
			tweener = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Light>().DOColor(to.Value, duration.Value);
			...
			if (debugThis.Value)
			{
				Debug.Log("GameObject [" + base.State.Fsm.GameObjectName + "] FSM [" + base.State.Fsm.Name + "]  State [" + base.State.Name + "] - DOTween Light Color - SUCCESS!");
			}
			if (finishImmediately.Value)
			{
				Finish();
