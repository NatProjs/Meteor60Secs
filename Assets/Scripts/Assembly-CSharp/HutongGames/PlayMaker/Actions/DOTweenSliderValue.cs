...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenSliderValue : FsmStateAction
	{
		...
		{
			tweener = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Slider>().DOValue(to.Value, duration.Value, snapping.Value);
			...
			if (debugThis.Value)
			{
				Debug.Log("GameObject [" + base.State.Fsm.GameObjectName + "] FSM [" + base.State.Fsm.Name + "] State [" + base.State.Name + "] - DOTween Slider Value - SUCCESS!");
			}
			if (finishImmediately.Value)
			{
				Finish();
