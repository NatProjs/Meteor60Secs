...
namespace Com.LuisPedroFonseca.ProCamera2D
{
	...
	public class ProCamera2DForwardFocus : BasePC2D, IPreMover
	{
		...
		private void ApplyInfluence(float deltaTime)
		{
			...
			{
				num = ((Mathf.Abs(num) < 0.001f) ? 0f : (((!(num < 0f)) ? RightFocus : (0f - LeftFocus)) * base.ProCamera2D.ScreenSizeInWorldCoordinates.x));
				num2 = ((Mathf.Abs(num2) < 0.001f) ? 0f : (((!(num2 < 0f)) ? TopFocus : (0f - BottomFocus)) * base.ProCamera2D.ScreenSizeInWorldCoordinates.y));
			}
			else
			{
				bool flag;
				...
				{
					_targetHVel = ((!(num < 0f)) ? RightFocus : (0f - LeftFocus)) * base.ProCamera2D.ScreenSizeInWorldCoordinates.x;
				}
				num = _targetHVel;
				bool flag2;
				...
				{
					_targetVVel = ((!(num2 < 0f)) ? TopFocus : (0f - BottomFocus)) * base.ProCamera2D.ScreenSizeInWorldCoordinates.y;
				}
				num2 = _targetVVel;
			}
