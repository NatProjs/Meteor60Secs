...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenRectTransformShakeAnchorPos : FsmStateAction
	{
		...
		{
			tweener = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<RectTransform>().DOShakeAnchorPos(duration.Value, strength.Value, vibrato.Value, randomness.Value, snapping.Value);
			...
			if (debugThis.Value)
			{
				Debug.Log("GameObject [" + base.State.Fsm.GameObjectName + "] FSM [" + base.State.Fsm.Name + "] State [" + base.State.Name + "] - DOTween RectTransform Shake Anchor Pos - SUCCESS!");
			}
			if (finishImmediately.Value)
			{
				Finish();
