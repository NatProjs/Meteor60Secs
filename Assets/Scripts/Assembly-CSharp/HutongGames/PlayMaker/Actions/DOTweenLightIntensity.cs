...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenLightIntensity : FsmStateAction
	{
		...
		{
			tweener = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Light>().DOIntensity(to.Value, duration.Value);
			...
			if (debugThis.Value)
			{
				Debug.Log("GameObject [" + base.State.Fsm.GameObjectName + "] FSM [" + base.State.Fsm.Name + "] State [" + base.State.Name + "] - DOTween Light Intensity - SUCCESS!");
			}
			if (finishImmediately.Value)
			{
				Finish();
