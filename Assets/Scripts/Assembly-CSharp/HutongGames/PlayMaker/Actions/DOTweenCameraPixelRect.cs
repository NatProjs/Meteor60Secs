...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenCameraPixelRect : FsmStateAction
	{
		...
		{
			tweener = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Camera>().DOPixelRect(to.Value, duration.Value);
			...
			if (debugThis.Value)
			{
				Debug.Log("GameObject [" + base.State.Fsm.GameObjectName + "] FSM [" + base.State.Fsm.Name + "]  State [" + base.State.Name + "] - DOTween Camera Pixel Rect - SUCCESS!");
			}
			if (finishImmediately.Value)
			{
				Finish();
