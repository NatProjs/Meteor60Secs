...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenTextText : FsmStateAction
	{
		...
		{
			tweener = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Text>().DOText(to.Value, duration.Value, richTextEnabled.Value, scrambleMode, scrambleChars.Value);
			...
			if (debugThis.Value)
			{
				Debug.Log("GameObject [" + base.State.Fsm.GameObjectName + "] FSM [" + base.State.Fsm.Name + "] State [" + base.State.Name + "] - DOTween Text Text - SUCCESS!");
			}
			if (finishImmediately.Value)
			{
				Finish();
