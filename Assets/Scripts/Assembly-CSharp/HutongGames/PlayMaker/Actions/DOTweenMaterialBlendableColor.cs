...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenMaterialBlendableColor : FsmStateAction
	{
		...
		{
			tweener = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Renderer>().material.DOBlendableColor(to.Value, duration.Value);
			...
			if (debugThis.Value)
			{
				Debug.Log("GameObject [" + base.State.Fsm.GameObjectName + "] FSM [" + base.State.Fsm.Name + "] State [" + base.State.Name + "] - DOTween Material Blendable Color - SUCCESS!");
			}
			if (finishImmediately.Value)
			{
				Finish();
