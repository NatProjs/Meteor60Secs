...
namespace Com.LuisPedroFonseca.ProCamera2D
{
	...
	public class ProCamera2D : MonoBehaviour, ISerializationCallbackReceiver
	{
		...
		public bool IsMoving
		{
			get
			{
				return Vector3H(_transform.localPosition) != Vector3H(_previousCameraPosition) || Vector3V(_transform.localPosition) != Vector3V(_previousCameraPosition);
			}
		}
		...
		public Vector3 InfluencesSum
		{
			get
			{
				return _influencesSum;
			}
		}
		...
		private void Start()
		{
			...
			if (CenterTargetOnStart && CameraTargets.Count > 0)
			{
				...
				float x = ((!FollowHorizontal) ? Vector3H(_transform.localPosition) : Vector3H(targetsWeightedMidPoint));
				float y = ((!FollowVertical) ? Vector3V(_transform.localPosition) : Vector3V(targetsWeightedMidPoint));
		...
		public CameraTarget AddCameraTarget(Transform targetTransform, float targetInfluenceH = 1f, float targetInfluenceV = 1f, float duration = 0f, Vector2 targetOffset = default(Vector2))
		{
			...
			cameraTarget.TargetOffset = targetOffset;
			CameraTarget cameraTarget2 = cameraTarget;
			CameraTargets.Add(cameraTarget2);
			if (duration > 0f)
			{
				cameraTarget2.TargetInfluence = 0f;
				StartCoroutine(AdjustTargetInfluenceRoutine(cameraTarget2, targetInfluenceH, targetInfluenceV, duration));
			}
			return cameraTarget2;
		}
		...
		public void RemoveCameraTarget(Transform targetTransform, float duration = 0f)
		{
			for (int i = 0; i < CameraTargets.Count; i++)
			{
				if (CameraTargets[i].TargetTransform.GetInstanceID() == targetTransform.GetInstanceID())
				{
					if (duration > 0f)
					{
						StartCoroutine(AdjustTargetInfluenceRoutine(CameraTargets[i], 0f, 0f, duration, true));
					}
					else
					{
		...
		public void Move(float deltaTime)
		{
			...
			if (!(DeltaTime < 0.0001f))
			{
				...
				_influences.Clear();
				float num = ((!FollowHorizontal) ? Vector3H(_transform.localPosition) : Vector3H(CameraTargetPosition));
				float num2 = ((!FollowVertical) ? Vector3V(_transform.localPosition) : Vector3V(CameraTargetPosition));
				...
				CameraTargetPosition += VectorHV((!FollowHorizontal) ? 0f : GetOffsetX(), (!FollowVertical) ? 0f : GetOffsetY());
		...
		private IEnumerator ApplyInfluencesTimedRoutine(IList<Vector2> influences, float[] durations)
		{
			...
			{
				count++;
				yield return StartCoroutine(ApplyInfluenceTimedRoutine(duration: durations[count], influence: influences[count]));
			}
		}
		...
		{
			return (!IsRelativeOffset) ? OffsetX : (OffsetX * ScreenSizeInWorldCoordinates.x * 0.5f);
		}

		private float GetOffsetY()
		{
			return (!IsRelativeOffset) ? OffsetY : (OffsetY * ScreenSizeInWorldCoordinates.y * 0.5f);
		}
