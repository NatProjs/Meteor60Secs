...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenRectTransformSizeDelta : FsmStateAction
	{
		...
		{
			tweener = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<RectTransform>().DOSizeDelta(to.Value, duration.Value, snapping.Value);
			...
			if (debugThis.Value)
			{
				Debug.Log("GameObject [" + base.State.Fsm.GameObjectName + "] FSM [" + base.State.Fsm.Name + "] State [" + base.State.Name + "] - DOTween RectTransform Size Delta - SUCCESS!");
			}
			if (finishImmediately.Value)
			{
				Finish();
