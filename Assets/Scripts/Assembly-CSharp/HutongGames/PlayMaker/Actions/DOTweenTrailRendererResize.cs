...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenTrailRendererResize : FsmStateAction
	{
		...
		public override void Reset()
		{
			base.Reset();
			gameObject = null;
			toStartWidth = new FsmFloat
			{
				UseVariable = false
			};
			toEndWidth = new FsmFloat
			{
				UseVariable = false
			};
		...
		{
			tweener = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<TrailRenderer>().DOResize(toStartWidth.Value, toEndWidth.Value, duration.Value);
			...
			if (debugThis.Value)
			{
				Debug.Log("GameObject [" + base.State.Fsm.GameObjectName + "] FSM [" + base.State.Fsm.Name + "] State [" + base.State.Name + "] - DOTween Trail Renderer Resize - SUCCESS!");
			}
			if (finishImmediately.Value)
			{
				Finish();
