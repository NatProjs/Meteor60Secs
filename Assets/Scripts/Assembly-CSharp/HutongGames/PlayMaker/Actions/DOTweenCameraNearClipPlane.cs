...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenCameraNearClipPlane : FsmStateAction
	{
		...
		{
			tweener = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Camera>().DONearClipPlane(to.Value, duration.Value);
			...
			if (debugThis.Value)
			{
				Debug.Log("GameObject [" + base.State.Fsm.GameObjectName + "] FSM [" + base.State.Fsm.Name + "]  State [" + base.State.Name + "] - DOTween Camera Near Clip Plane - SUCCESS!");
			}
			if (finishImmediately.Value)
			{
				Finish();
