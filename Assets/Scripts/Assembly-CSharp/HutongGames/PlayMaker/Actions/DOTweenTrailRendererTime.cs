...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenTrailRendererTime : FsmStateAction
	{
		...
		{
			tweener = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<TrailRenderer>().DOTime(to.Value, duration.Value);
			...
			if (debugThis.Value)
			{
				Debug.Log("GameObject [" + base.State.Fsm.GameObjectName + "] FSM [" + base.State.Fsm.Name + "] State [" + base.State.Name + "] - DOTween Trail Renderer Time - SUCCESS!");
			}
			if (finishImmediately.Value)
			{
				Finish();
