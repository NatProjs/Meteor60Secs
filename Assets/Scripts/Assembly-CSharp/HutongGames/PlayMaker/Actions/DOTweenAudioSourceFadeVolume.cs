...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenAudioSourceFadeVolume : FsmStateAction
	{
		...
		{
			tweener = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<AudioSource>().DOFade(to.Value, duration.Value);
			...
			if (debugThis.Value)
			{
				Debug.Log("GameObject [" + base.State.Fsm.GameObjectName + "] FSM [" + base.State.Fsm.Name + "] State [" + base.State.Name + "] - DOTween AudioSource Fade Volume - SUCCESS!");
			}
			if (finishImmediately.Value)
			{
				Finish();
