...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenRectTransformAnchorPos3D : FsmStateAction
	{
		...
		{
			tweener = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<RectTransform>().DOAnchorPos3D(to.Value, duration.Value, snapping.Value);
			...
			if (debugThis.Value)
			{
				Debug.Log("GameObject [" + base.State.Fsm.GameObjectName + "] FSM [" + base.State.Fsm.Name + "] State [" + base.State.Name + "] - DOTween RectTransform Anchor Pos 3D - SUCCESS!");
			}
			if (finishImmediately.Value)
			{
				Finish();
