	...
	public class ProCamera2DShake : BasePC2D
	{
		...
		protected override void Awake()
		{
			...
			else
			{
				...
				base.ProCamera2D.transform.parent = transform;
				_shakeParent = transform;
			}
		...
		public void Shake(float duration, Vector2 strength, int vibrato = 10, float randomness = 0.1f, float initialAngle = -1f, Vector3 rotation = default(Vector3), float smoothness = 0.1f, bool ignoreTimeScale = false)
		{
			...
			for (int k = 0; k < vibrato - 1; k++)
			{
				...
				array3[k] = ((num8 != 1) ? (Quaternion.Inverse(Quaternion.Lerp(a, Quaternion.identity, t)) * _originalRotation) : (Quaternion.Lerp(a, Quaternion.identity, t) * _originalRotation));
			}
		...
		{
			Shake(preset.Duration, preset.Strength, preset.Vibrato, preset.Randomness, (!preset.UseRandomInitialAngle) ? preset.InitialAngle : (-1f), preset.Rotation, preset.Smoothness, preset.IgnoreTimeScale);
		}
		...
		private IEnumerator ShakeRoutine(float smoothness, bool ignoreTimeScale = false)
		{
			...
			{
				Vector3 newShakePosition = Utils.GetVectorsSum(_shakePositions) + _influencesSum;
				Vector3 newShakePositionSmoothed = Vector3.zero;
				if (ignoreTimeScale)
				{
				...
				Quaternion rotationTargetSmoothed = Quaternion.Slerp(_transform.localRotation, _rotationTarget, _rotationTime);
		...
		private IEnumerator ApplyShakesTimedRoutine(IList<Vector2> shakes, IList<Quaternion> rotations, float[] durations, bool ignoreTimeScale = false)
		{
			...
			{
				count++;
				yield return StartCoroutine(ApplyShakeTimedRoutine(duration: durations[count], shake: shakes[count], rotation: rotations[count], ignoreTimeScale: ignoreTimeScale));
			}
		}
		...
		private IEnumerator CalculateConstantShakePosition(int index, float frequencyMin, float frequencyMax, float amplitudeX, float amplitudeY, float amplitudeZ)
		{
			...
			{
				float randomFrequency = UnityEngine.Random.Range(frequencyMin, frequencyMax);
				Vector3 unitSphere = UnityEngine.Random.insideUnitSphere;
				float randomAmplitudeX = unitSphere.x * amplitudeX;
				float randomAmplitudeY = unitSphere.y * amplitudeY;
				float randomAmplitudeZ = unitSphere.z * amplitudeZ;
