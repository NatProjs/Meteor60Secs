...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenTextColor : FsmStateAction
	{
		...
		{
			tweener = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Text>().DOColor(to.Value, duration.Value);
			...
			if (debugThis.Value)
			{
				Debug.Log("GameObject [" + base.State.Fsm.GameObjectName + "] FSM [" + base.State.Fsm.Name + "] State [" + base.State.Name + "] - DOTween Text Color - SUCCESS!");
			}
			if (finishImmediately.Value)
			{
				Finish();
