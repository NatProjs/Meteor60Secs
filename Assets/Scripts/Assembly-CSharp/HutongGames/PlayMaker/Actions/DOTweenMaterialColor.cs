...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenMaterialColor : FsmStateAction
	{
		...
		{
			tweener = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Renderer>().material.DOColor(to.Value, duration.Value);
			...
			if (debugThis.Value)
			{
				Debug.Log("GameObject [" + base.State.Fsm.GameObjectName + "] FSM [" + base.State.Fsm.Name + "] State [" + base.State.Name + "] - DOTween Material Color - SUCCESS!");
			}
			if (finishImmediately.Value)
			{
				Finish();
