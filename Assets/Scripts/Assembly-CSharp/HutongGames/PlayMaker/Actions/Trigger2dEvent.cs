...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class Trigger2dEvent : FsmStateAction
	{
		...
		{
			if (trigger == Trigger2DType.OnTriggerEnter2D && other.gameObject.tag == collideTag.Value)
			{
		...
		{
			if (trigger == Trigger2DType.OnTriggerStay2D && other.gameObject.tag == collideTag.Value)
			{
				StoreCollisionInfo(other);
				base.Fsm.Event(sendEvent);
			}
		...
		{
			if (trigger == Trigger2DType.OnTriggerExit2D && other.gameObject.tag == collideTag.Value)
			{
				StoreCollisionInfo(other);
				base.Fsm.Event(sendEvent);
			}
