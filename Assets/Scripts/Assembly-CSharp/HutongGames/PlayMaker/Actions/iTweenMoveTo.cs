	...
	public class iTweenMoveTo : iTweenFsmAction
	{
		...
		public override void OnDrawActionGizmos()
		{
			...
			for (int i = 0; i < transforms.Length; i++)
			{
				...
				{
					tempVct3[i] = ((!vectors[i].IsNone) ? vectors[i].Value : Vector3.zero);
				}
				...
				{
					tempVct3[i] = transforms[i].Value.transform.position + ((!vectors[i].IsNone) ? vectors[i].Value : Vector3.zero);
				}
			}
		...
		private void DoiTween()
		{
			...
			{
				vector = ((space != 0 && !(ownerDefaultTarget.transform.parent == null)) ? (ownerDefaultTarget.transform.parent.InverseTransformPoint(transformPosition.Value.transform.position) + vector) : (transformPosition.Value.transform.position + vector));
			}
			...
			if (transforms.Length >= 2)
			{
				...
				if (!reverse.IsNone && reverse.Value)
				{
					for (int i = 0; i < transforms.Length; i++)
					{
						if (transforms[i].IsNone)
						{
							tempVct3[tempVct3.Length - 1 - i] = ((!vectors[i].IsNone) ? vectors[i].Value : Vector3.zero);
						}
						else if (transforms[i].Value == null)
						{
							tempVct3[tempVct3.Length - 1 - i] = ((!vectors[i].IsNone) ? vectors[i].Value : Vector3.zero);
						...
						{
							tempVct3[tempVct3.Length - 1 - i] = ((space != 0) ? transforms[i].Value.transform.localPosition : transforms[i].Value.transform.position) + ((!vectors[i].IsNone) ? vectors[i].Value : Vector3.zero);
						}
					}
				}
				else
				{
					for (int j = 0; j < transforms.Length; j++)
					{
						...
						{
							tempVct3[j] = ((!vectors[j].IsNone) ? vectors[j].Value : Vector3.zero);
						}
						...
						{
							tempVct3[j] = ((space != 0) ? ownerDefaultTarget.transform.parent.InverseTransformPoint(transforms[j].Value.transform.position) : transforms[j].Value.transform.position) + ((!vectors[j].IsNone) ? vectors[j].Value : Vector3.zero);
						}
					}
				}
				hashtable.Add("path", tempVct3);
				hashtable.Add("movetopath", moveToPath.IsNone || moveToPath.Value);
				hashtable.Add("lookahead", (!lookAhead.IsNone) ? lookAhead.Value : 1f);
			}
			itweenType = "move";
