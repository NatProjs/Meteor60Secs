...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenImageFillAmount : FsmStateAction
	{
		...
		{
			tweener = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Image>().DOFillAmount(to.Value, duration.Value);
			...
			if (debugThis.Value)
			{
				Debug.Log("GameObject [" + base.State.Fsm.GameObjectName + "] FSM [" + base.State.Fsm.Name + "] State [" + base.State.Name + "] - DOTween Image Fill Amount - SUCCESS!");
			}
			if (finishImmediately.Value)
			{
				Finish();
