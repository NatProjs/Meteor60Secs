...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenAnimateInt : FsmStateAction
	{
		...
		{
			tweener = DOTween.To(() => variable.Value, delegate(int x)
			{
				variable.Value = x;
			...
			if (debugThis.Value)
			{
				Debug.Log("GameObject [" + base.State.Fsm.GameObjectName + "] FSM [" + base.State.Fsm.Name + "] State [" + base.State.Name + "] - DOTween Animate Int - SUCCESS!");
			}
			if (finishImmediately.Value)
			{
				Finish();
