...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenOutlineColor : FsmStateAction
	{
		...
		{
			tweener = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Outline>().DOColor(to.Value, duration.Value);
			...
			if (debugThis.Value)
			{
				Debug.Log("GameObject [" + base.State.Fsm.GameObjectName + "] FSM [" + base.State.Fsm.Name + "] State [" + base.State.Name + "] - DOTween Outline Color - SUCCESS!");
			}
			if (finishImmediately.Value)
			{
				Finish();
