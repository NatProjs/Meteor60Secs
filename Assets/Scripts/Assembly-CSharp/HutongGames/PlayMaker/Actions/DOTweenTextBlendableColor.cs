...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenTextBlendableColor : FsmStateAction
	{
		...
		{
			tweener = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Text>().DOBlendableColor(to.Value, duration.Value);
			...
			if (debugThis.Value)
			{
				Debug.Log("GameObject [" + base.State.Fsm.GameObjectName + "] FSM [" + base.State.Fsm.Name + "] State [" + base.State.Name + "] - DOTween Text Blendable Color - SUCCESS!");
			}
			if (finishImmediately.Value)
			{
				Finish();
