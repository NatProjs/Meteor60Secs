...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenImageFade : FsmStateAction
	{
		...
		{
			tweener = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Image>().DOFade(to.Value, duration.Value);
			...
			if (debugThis.Value)
			{
				Debug.Log("GameObject [" + base.State.Fsm.GameObjectName + "] FSM [" + base.State.Fsm.Name + "] State [" + base.State.Name + "] - DOTween Image Fade - SUCCESS!");
			}
			if (finishImmediately.Value)
			{
				Finish();
