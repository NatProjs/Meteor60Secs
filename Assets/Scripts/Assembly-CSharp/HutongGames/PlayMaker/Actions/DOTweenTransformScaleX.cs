	...
	public class DOTweenTransformScaleX : FsmStateAction
	{
		...
		public override void OnEnter()
		{
			...
			case DOTweenActionsEnums.Target.Value:
				tweener = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Transform>().DOScaleX(to.Value, duration.Value);
				break;
			case DOTweenActionsEnums.Target.GameObject:
				tweener = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Transform>().DOScaleX(toGameObject.Value.transform.localScale.x, duration.Value);
				break;
			}
			...
			if (debugThis.Value)
			{
				Debug.Log("GameObject [" + base.State.Fsm.GameObjectName + "] FSM [" + base.State.Fsm.Name + "] State [" + base.State.Name + "] - DOTween Transform Scale X - SUCCESS!");
			}
			if (finishImmediately.Value)
			{
				Finish();
