	...
	public class ProCamera2DTriggerInfluence : BaseTrigger
	{
		...
		private IEnumerator InsideTriggerRoutine()
		{
			...
			while (_insideTrigger)
			{
				...
				float distancePercentage = GetDistanceToCenterPercentage(new Vector2(Vector3H(base.ProCamera2D.TargetsMidPoint), Vector3V(base.ProCamera2D.TargetsMidPoint)));
				Vector2 vectorFromPointToFocus = new Vector2(Vector3H(base.ProCamera2D.TargetsMidPoint) + Vector3H(base.ProCamera2D.TargetsMidPoint) - Vector3H(base.ProCamera2D.PreviousTargetsMidPoint), Vector3V(base.ProCamera2D.TargetsMidPoint) + Vector3V(base.ProCamera2D.TargetsMidPoint) - Vector3V(base.ProCamera2D.PreviousTargetsMidPoint)) - new Vector2(Vector3H(FocusPoint.position), Vector3V(FocusPoint.position));
				if (distancePercentage == 0f)
				{
					...
					_influence = -vectorFromPointToFocus * (1f - distancePercentage);
				...
				else
				{
					...
					_influence = Vector2.SmoothDamp(_influence, -vectorFromPointToFocus * (1f - distancePercentage), ref _velocity, InfluenceSmoothness, float.PositiveInfinity, Time.deltaTime);
					base.ProCamera2D.ApplyInfluence(_influence);
