...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenAnimateColor : FsmStateAction
	{
		...
		{
			tweener = DOTween.To(() => variable.Value, delegate(Color x)
			{
				variable.Value = x;
			...
			if (debugThis.Value)
			{
				Debug.Log("GameObject [" + base.State.Fsm.GameObjectName + "] FSM [" + base.State.Fsm.Name + "] State [" + base.State.Name + "] - DOTween Animate Color - SUCCESS!");
			}
			if (finishImmediately.Value)
			{
				Finish();
