	...
	public abstract class BaseTrigger : BasePC2D
	{
		...
		protected float GetDistanceToCenterPercentage(Vector2 point)
		{
			...
			if (TriggerShape == TriggerShape.RECTANGLE)
			{
				...
				float f2 = Vector3V(_vectorFromPointToCenter) / (Vector3V(_transform.localScale) * 0.5f);
				return Mathf.Max(Mathf.Abs(f), Mathf.Abs(f2)).Remap(_exclusiveInfluencePercentage, 1f, 0f, 1f);
			}
