...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenMaterialFadeProperty : FsmStateAction
	{
		...
		{
			tweener = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Renderer>().material.DOFade(to.Value, property.Value, duration.Value);
			...
			if (debugThis.Value)
			{
				Debug.Log("GameObject [" + base.State.Fsm.GameObjectName + "] FSM [" + base.State.Fsm.Name + "] State [" + base.State.Name + "] - DOTween Material Fade Property - SUCCESS!");
			}
			if (finishImmediately.Value)
			{
				Finish();
