...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenRigidbodyLookAtPosition : FsmStateAction
	{
		...
		{
			tweener = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Rigidbody>().DOLookAt(position.Value, duration.Value, axisConstraint, up.Value);
			...
			if (debugThis.Value)
			{
				Debug.Log("GameObject [" + base.State.Fsm.GameObjectName + "] FSM [" + base.State.Fsm.Name + "] State [" + base.State.Name + "] - DOTween RigidBody Look At Position - SUCCESS!");
			}
			if (finishImmediately.Value)
			{
				Finish();
