...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenLayoutElementMinSize : FsmStateAction
	{
		...
		{
			tweener = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<LayoutElement>().DOMinSize(to.Value, duration.Value, snapping.Value);
			...
			if (debugThis.Value)
			{
				Debug.Log("GameObject [" + base.State.Fsm.GameObjectName + "] FSM [" + base.State.Fsm.Name + "] State [" + base.State.Name + "] - DOTween Layout Element Min Size - SUCCESS!");
			}
			if (finishImmediately.Value)
			{
				Finish();
