...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenSpriteRendererColor : FsmStateAction
	{
		...
		{
			tweener = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<SpriteRenderer>().DOColor(to.Value, duration.Value);
			...
			if (debugThis.Value)
			{
				Debug.Log("GameObject [" + base.State.Fsm.GameObjectName + "] FSM [" + base.State.Fsm.Name + "] State [" + base.State.Name + "] - DOTween Sprite Renderer Color - SUCCESS!");
			}
			if (finishImmediately.Value)
			{
				Finish();
