...
namespace Com.LuisPedroFonseca.ProCamera2D
{
	...
	public class ProCamera2DRepeater : BasePC2D, IPostMover
	{
			...
			for (int i = startIndex.X; i <= endIndex.X; i++)
			{
				for (int j = startIndex.Y; j <= endIndex.Y; j++)
				{
					...
					if (!_occupiedIndices.TryGetValue(intPoint, out value))
					{
						...
						{
							InitCopy(Object.Instantiate(ObjectToRepeat), false);
						}
		...
		private void InitCopy(Transform newCopy, bool positionOffscreen = true)
		{
			...
			RepeatedObject repeatedObject2 = repeatedObject;
			repeatedObject2.Transform.parent = ObjectToRepeat.parent;
			_allRepeatedObjects.Add(repeatedObject2);
			_inactiveRepeatedObjects.Enqueue(repeatedObject2);
			if (positionOffscreen)
			{
				PositionObject(repeatedObject2, IntPoint.MaxValue);
			}
		}
