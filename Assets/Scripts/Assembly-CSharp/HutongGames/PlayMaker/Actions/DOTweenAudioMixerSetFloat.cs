...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenAudioMixerSetFloat : FsmStateAction
	{
		...
		public string descriptionArea = "Drag and Drop an AudioMixer from the ProjectView";

		[RequiredField]
		[Tooltip("The AudioMixer you want to control")]
		public AudioMixer audioMixer;

		[RequiredField]
		[UIHint(UIHint.FsmFloat)]
		[Tooltip("Exposed parameter name of type float")]
		...
		public override void OnEnter()
		{
			if (audioMixer == null)
			{
				if (debugThis.Value)
				{
					Debug.LogError("GameObject [" + base.State.Fsm.GameObjectName + "] FSM [" + base.State.Fsm.Name + "] State [" + base.State.Name + "] - DOTween AudioMixer SetFloat - ERROR - There is no AudioMixer linked to this state");
				}
				return;
			}
			tweener = audioMixer.DOSetFloat(floatName.Value, to.Value, duration.Value);
			...
			if (debugThis.Value)
			{
				Debug.Log("GameObject [" + base.State.Fsm.GameObjectName + "] FSM [" + base.State.Fsm.Name + "] State [" + base.State.Name + "] - DOTween AudioMixer SetFloat - SUCCESS!");
			}
			if (finishImmediately.Value)
			{
				Finish();
