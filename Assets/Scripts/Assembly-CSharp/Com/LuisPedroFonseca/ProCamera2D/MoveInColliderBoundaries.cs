...
namespace Com.LuisPedroFonseca.ProCamera2D
{
	public class MoveInColliderBoundaries
	{
		...
		public Vector3 Move(Vector3 deltaMovement)
		{
			...
			{
				arg = MoveInAxis(Vector3H(deltaMovement), true);
			}
			float arg2 = 0f;
			if (Vector3V(deltaMovement) != 0f)
			{
				arg2 = MoveInAxis(Vector3V(deltaMovement), false);
			}
			...
			if (Physics.Raycast(vector, normalized, out _raycastHit, num, CameraCollisionMask))
			{
				if (Mathf.Abs(Vector3H(_raycastHit.normal)) > Mathf.Abs(Vector3V(_raycastHit.normal)))
				{
					...
					if (Vector3H(deltaMovement) == 0f)
					{
						...
						float arg2 = MoveInAxis(Vector3H(deltaMovement), true);
				...
				else
				{
					...
					if (Vector3V(deltaMovement) == 0f)
					{
						...
						float arg4 = MoveInAxis(Vector3V(deltaMovement), false);
		...
		private float MoveInAxis(float deltaMovement, bool isHorizontal)
		{
			...
			if (isHorizontal)
			{
				...
				arg = ((!flag) ? _raycastOrigins.BottomLeft : _raycastOrigins.BottomRight);
			}
			else
			{
				vector = ((!flag) ? (-CameraTransform.up) : CameraTransform.up);
				arg = ((!flag) ? _raycastOrigins.BottomLeft : _raycastOrigins.TopLeft);
			}
			float num2 = float.NegativeInfinity;
			bool flag2 = false;
			int num3 = ((!isHorizontal) ? TotalVerticalRays : TotalHorizontalRays);
			for (int i = 0; i < num3; i++)
			{
				float num4 = ((!isHorizontal) ? (Vector3H(arg) + (float)i * _horizontalDistanceBetweenRays) : Vector3H(arg));
				float num5 = ((!isHorizontal) ? Vector3V(arg) : (Vector3V(arg) + (float)i * _verticalDistanceBetweenRays));
				if (isHorizontal)
				{
				...
				if (Physics.Raycast(vector2, vector, out _raycastHit, num, CameraCollisionMask))
				{
					...
					{
						flag2 = true;
						deltaMovement = ((!isHorizontal) ? (Vector3V(_raycastHit.point) - Vector3V(vector2) + ((!flag) ? 0.2f : (-0.2f))) : (Vector3H(_raycastHit.point) - Vector3H(vector2) + ((!flag) ? 0.2f : (-0.2f))));
						num2 = ((!isHorizontal) ? Vector3V(_raycastHit.point) : Vector3H(_raycastHit.point));
					}
				}
				else
				{
