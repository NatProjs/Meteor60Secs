	...
	public class EnemySight : MonoBehaviour
	{
		...
		private IEnumerator Start()
		{
			while (true)
			{
				...
				if (angle < fieldOfViewAngle * 0.5f && Physics.Raycast(base.transform.position + base.transform.up, direction.normalized, out _hit, ViewDistance, LayerMask) && _hit.collider.transform.GetInstanceID() == player.GetInstanceID())
				{
					if (!playerInSight)
					{
						playerInSight = true;
