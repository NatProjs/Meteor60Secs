		...
		private void Move()
		{
			...
			for (int i = 0; i < ParallaxLayers.Count; i++)
			{
				if (ParallaxLayers[i].CameraTransform != null)
				{
					...
					float arg2 = ((!ParallaxHorizontal) ? Vector3H(arg) : (Vector3H(arg) * ParallaxLayers[i].Speed));
					float arg3 = ((!ParallaxVertical) ? Vector3V(arg) : (Vector3V(arg) * ParallaxLayers[i].Speed));
