...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenMaterialFloatProperty : FsmStateAction
	{
		...
		{
			tweener = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Renderer>().material.DOFloat(to.Value, property.Value, duration.Value);
			...
			if (debugThis.Value)
			{
				Debug.Log("GameObject [" + base.State.Fsm.GameObjectName + "] FSM [" + base.State.Fsm.Name + "] State [" + base.State.Name + "] - DOTween Material Float Property - SUCCESS!");
			}
			if (finishImmediately.Value)
			{
				Finish();
