...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class MoveObject : EaseFsmAction
	{
		...
		public override void OnUpdate()
		{
			...
			{
				ownerDefaultTarget.transform.position = new Vector3(reverse.IsNone ? toVector.Value.x : ((!reverse.Value) ? toVector.Value.x : fromValue.Value.x), reverse.IsNone ? toVector.Value.y : ((!reverse.Value) ? toVector.Value.y : fromValue.Value.y), reverse.IsNone ? toVector.Value.z : ((!reverse.Value) ? toVector.Value.z : fromValue.Value.z));
