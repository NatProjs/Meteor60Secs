...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenRigidbodyLookAtGameObject : FsmStateAction
	{
		...
		{
			tweener = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Rigidbody>().DOLookAt(target.Value.transform.position, duration.Value, axisConstraint, up.Value);
			...
			if (debugThis.Value)
			{
				Debug.Log("GameObject [" + base.State.Fsm.GameObjectName + "] FSM [" + base.State.Fsm.Name + "] State [" + base.State.Name + "] - DOTween RigidBody Look At GameObject - SUCCESS!");
			}
			if (finishImmediately.Value)
			{
				Finish();
