	...
	public class DOTweenTransformJump : FsmStateAction
	{
		...
		public override void OnEnter()
		{
			...
			case DOTweenActionsEnums.Target.Value:
				sequence.Append(base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Transform>().DOJump(to.Value, jumpPower.Value, numJumps.Value, duration.Value, snapping.Value)
					.SetRelative(setRelative.Value));
				break;
			case DOTweenActionsEnums.Target.GameObject:
				sequence.Append(base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Transform>().DOJump(toGameObject.Value.transform.position, jumpPower.Value, numJumps.Value, duration.Value, snapping.Value)
					.SetRelative(setRelative.Value));
				break;
			}
			switch (tweenIdType)
			...
			if (debugThis.Value)
			{
				Debug.Log("GameObject [" + base.State.Fsm.GameObjectName + "] FSM [" + base.State.Fsm.Name + "] State [" + base.State.Name + "] - DOTween Transform Jump - SUCCESS!");
			}
			if (finishImmediately.Value)
			{
				Finish();
