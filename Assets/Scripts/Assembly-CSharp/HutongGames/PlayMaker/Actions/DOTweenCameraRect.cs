...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DOTweenCameraRect : FsmStateAction
	{
		...
		{
			tweener = base.Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Camera>().DORect(to.Value, duration.Value);
			...
			if (debugThis.Value)
			{
				Debug.Log("GameObject [" + base.State.Fsm.GameObjectName + "] FSM [" + base.State.Fsm.Name + "]  State [" + base.State.Name + "] - DOTween Camera Rect - SUCCESS!");
			}
			if (finishImmediately.Value)
			{
				Finish();
